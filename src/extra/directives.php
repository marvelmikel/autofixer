...
Directives::register([
     ...
     */
    'location-href' => function() : string 
    {
        // @var string $ref 
        $ref = '';

        // check for HTTP_REFERER in Server
        if (isset($_SERVER['HTTP_REFERER'])) :
            ...
            if (isset($_SERVER['REQUEST_URI'])) :
                ...
                // do we have ?
                if (strpos($url, '?') !== false) $ref .= substr($url, (strpos($url, '?')));
     ...
     * @return string
     */
    'load-cdn' => function(string $folder, string $file) : string 
    {
        // @var string $cdnUrl
        $cdnUrl = func()->finder('cdn.url');

        // use default url
        $cdnUrl = $cdnUrl == '' ? func()->url() . '/static/' : $cdnUrl;
    ...
    /**
     * @method get-launch-year
     * @return string
     * 
     * This would return the year this project was launched
     */
    'get-launch-year' => function() : string 
    {
        // @var string $year
        $year = func()->finder('launch_year');

        // get current year
        if (intval(date('Y')) > intval($year)) $year = $year . ' - ' . date('Y');

        // return string
        return $year;
    },

    'youtube-url' => function() : string 
    {
        return 'https://www.youtube.com/user/PaintballOO7';
    }
